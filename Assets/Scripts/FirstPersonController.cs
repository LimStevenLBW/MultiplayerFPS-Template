...
public class FirstPersonController : NetworkBehaviour
{
    ...
    public Transform CrossHairAndStamina;
    ...
    {
        Vector3 rollDirection = new Vector3(0, 0, 0);

        if (Input.GetKey(KeyCode.W)) rollDirection = new Vector3(2, 0, 0);
        if (Input.GetKey(KeyCode.A)) rollDirection = new Vector3(0, 0, 2);
        if (Input.GetKey(KeyCode.S)) rollDirection = new Vector3(-2, 0, 0);
        if (Input.GetKey(KeyCode.D)) rollDirection = new Vector3(0, 0, -2);
        if (rollDirection == new Vector3(0, 0, 0)) rollDirection = new Vector3(-2, 0, 0);

        Vector3 forceDirection = playerCamera.transform.parent.parent.transform.forward;
